...
**已补齐 C + D**(2026-07-04):
- **齿轮 C 共识代理**(`consensus.py`):给 `fre radar` 传 `--corpus <dated docs>`,按候选约束的
  `terms` 统计"近期 vs 早期"提及率 → 每个约束得 `{level, trend, pre_consensus}`;迁移告警据此标注:
  **"余量比侵蚀 + 提及仍 low/flat" → `pre_consensus=True`(前共识 alpha 窗口)**,"提及 rising" → 标注"likely already priced"。
  demo(`configs/radar/hbm4.corpus.json`):人人都在谈 HBM(rising=已定价)、开始谈 CoWoS,而**余量比最紧(0.65)的
  rack power/cooling 几乎无人提及 → pre-consensus**。
- **齿轮 D 雷达自校准**(`radar.py` 的 `radar_migration_predictions` / `register_radar_predictions`):
  `fre radar --register-predictions` 把每条迁移判断落成带日期的可证伪预测(action/investigate/watch → 概率 0.75/0.55/0.35),
  写入 `radar_state/<theme>.predictions.json`;报告内嵌 `calibration`(复用 `build_calibration`)。
